...
object Ex6
{
	def tennis(probP1ServeWin: Double, probP1Winner: Double, probP1NetError: Double, probP1OutError: Double, probP2ServeWin: Double, probP2Winner: Double, probP2NetError: Double, probP2OutError: Double): Element[Boolean] = 
	{
		def rally(firstShot: Boolean, player1: Boolean): Element[Boolean] = 
		{
			...
			val probP1Error = (probP1NetError + probP1OutError) / 2;
			val probP2Error = (probP2NetError + probP2OutError) / 2;
	...
	{
		val tennis_match = tennis(0.5, 0.5, 0.1, 0.1, 0.5, 0.5, 0.1, 0.1)
